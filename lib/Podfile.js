...
Podfile.prototype.getTemplateForPodsFrameworks = function () {
    // Escaping possible ' in the project name
    // const projectName = this.escapeSingleQuotes(this.projectName);
    return `
        post_install do |installer|
            podsFrameworksPath = "Pods/Target Support Files/Pods-${this.projectName}/Pods-${this.projectName}-frameworks.sh"

            if File.exist?(podsFrameworksPath)
                puts "Found #{podsFrameworksPath}"
                text = File.read(podsFrameworksPath)
                new_contents = text.gsub(/\'\$1\'/, "\\\"$1\\\"")

                File.open(podsFrameworksPath, "w") {|file| file.puts new_contents}
            end
        end
    `
};
...
Podfile.prototype.appendContent = function(content, toolOptions) {
    const podfile = fs.readFileSync(this.path, 'utf8');
    const updatedPodfile = podfile + '\n' + content;

    fs.writeFileSync(this.path, updatedPodfile, 'utf8');
...
Podfile.prototype.install = function (requirementsCheckerFunction) {
    ...
    const podsFrameworkTemplate = this.getTemplateForPodsFrameworks();

    return requirementsCheckerFunction()
        .then(toolOptions => this.before_install(toolOptions))
        .then(toolOptions => this.appendContent(podsFrameworkTemplate, toolOptions))
        .then(toolOptions => {
